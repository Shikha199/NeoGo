...
{
    public partial class BillOfSupply : Mahesh_Page
    {
        ...
        private void SetControlType()
        {
            try
            {
                ...
                SetValueType(TextType.AlphNumericField_4, SINV_BasicRate);
        ...
        private void RefreshGrid_HDR(string SearchCriteria = "")
        {
            try
            {
                ...
                var_SQLFilter = var_SQLFilter + " AND SINV_Category ='Bill Of Supply'";
        ...
        private void InitializeControls_HDR()
        {
            try
            {
                ...
                SINV_No.Text = MEF.GetRefNo(SessionUserData.Company.CompanyID, SessionUserData.Company.BranchID, SessionUserData.Company.YearID, "Bill Of Supply", "SINV", "SINV", "Yes");
                ...
                if (ErrorMessage.Length == 0)
                {
                    using (TransactionScope scope = new TransactionScope())
                    {
                        if (Convert.ToInt32(SINVHDR_ID.Text) == 0)
                        {
                            ...
                            ObjAT_SINV_HDR.SINV_Category = "Bill Of Supply";
                            ...
                            ObjAT_SINV_HDR.SINV_No = MEF.GenrateRefNo(SessionUserData.Company.CompanyID, SessionUserData.Company.BranchID, SessionUserData.Company.YearID, "Bill Of Supply", "INV", "INV", "Yes", "", "", "", 0);
        ...
        protected void btnPrint_Click(object sender, System.EventArgs e)
        {
            try
            {
                // MABL.AppVariable.PrintDosReport_Invoice(SINVHDR_ID.Text);
                //if ((Inv_TaxType.Text == "SGST") || (Inv_TaxType.Text == "IGST"))
                //{
                // //   Response.Write("<script type='text/javascript'>detailedresults=window.open('" + MABL.AppVariable.Project_Constant("RptBrowse") + "/ReportBrowser/GSTInvBrowser.aspx?InvoiceType=" + Inv_TaxType.Text + "&INVID=" + SINVHDR_ID.Text + "');</script>");
                //}
                //else
                //{
        ...
        protected void Fk_MItem_ID_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                SINV_BasicRate.Text = dt.Rows[0]["Item_Purchase_Rate"].ToString();
                SINV_QTY.Text = "1";
                SINV_DiscountRate.Text = "0";
