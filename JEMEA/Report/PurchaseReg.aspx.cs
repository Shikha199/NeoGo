...
{
    public partial class PurchaseReg : Mahesh_Page
    {
        ...
        private void RefreshGrid_HDR(string SearchCriteria = "")
        {
            try
            {
                ...
                dt = MABL.BL.AppRptDP.Rpt_Get_Purchase_Summary(var_SQLFilter, SearchCriteria);
                if (dt.Rows.Count > 0)
                {
        ...
        private void RefreshGrid_Item_DTL(string SearchCriteria = "")
        {
            try
            {
                ...
                DataTable dt = default(DataTable);
                dt = MABL.BL.AppRptDP.Rpt_Get_Purchase_Detail(var_SQLFilter, SearchCriteria);
                if (dt.Rows.Count > 0)
                {
                    gvdtl.DataSource = dt;
                    gvdtl.DataBind();
        ...
        private void ExportToExcel_ExecutiveWiseSales()
        {
            try
            {
                ...
                Response.AddHeader("content-disposition", string.Format("attachment; filename={0}", "PurchaseReg_" + CurrDateTime + ".xls"));
                ...
                DataTable dt = default(DataTable);
                dt = MABL.BL.AppRptDP.Rpt_Get_Purchase_Summary(var_SQLFilter, "");
                if (dt.Rows.Count > 0)
                {
        ...
        private void ExportToExcelDetail_ExecutiveWiseSales()
        {
            try
            {
                ...
                DataTable dt = default(DataTable);
                dt = MABL.BL.AppRptDP.Rpt_Get_Purchase_Detail(var_SQLFilter, "");
                if (dt.Rows.Count > 0)
                {
