...
{
    public partial class SalesReg : Mahesh_Page
    {
        ...
        private void RefreshGrid_HDR(string SearchCriteria = "")
        {
            try
            {
                ...
                dt = MABL.BL.AppRptDP.Rpt_Get_Sales_Summary(var_SQLFilter, SearchCriteria);
                if (dt.Rows.Count > 0)
                {
        ...
        private void RefreshGrid_Item_DTL(string SearchCriteria = "")
        {
            try
            {
                ...
                {
                    var_SQLFilter =var_SQLFilter+ " AND SINV_DT BETWEEN  '" + Dtl_Txt_FromDT.Text + "' AND  '" + Dtl_Txt_ToDT.Text + "'";
                }

                DataTable dt = default(DataTable);
                dt = MABL.BL.AppRptDP.Rpt_Get_Sales_Detail(var_SQLFilter, SearchCriteria);
                if (dt.Rows.Count > 0)
                {
                    gvdtl.DataSource = dt;
                    gvdtl.DataBind();
        ...
        #endregion
        #region "Save"
        protected void btnExportToExcelRpt_Click(object sender, System.EventArgs e)
        {
            try
            {
                ExportToExcel_ExecutiveWiseSales();
            }
            catch (Exception ex)
            {
        ...
        protected void btnExportDetailToExcelRpt_Click(object sender, System.EventArgs e)
        {
            try
            {
                ExportToExcelDetail_ExecutiveWiseSales();
            }
            catch (Exception ex)
            {
                DisplayCustomMessageSummary("Failed To Generate Report Data");
        ...
        #endregion
        #region "Function"
        private void ExportToExcel_ExecutiveWiseSales()
        {
            try
            {
                string CurrDateTime = MaheshAF18.Utility.DateHelper.ConvertToDataBase(MaheshAF18.Utility.DateHelper.ConvertToForm(DateTime.Now)) +
                                      " " + DateTime.Now.Hour.ToString() + "-" + DateTime.Now.Minute.ToString() + "-" + DateTime.Now.Second.ToString();

                Response.ClearContent();
                Response.Buffer = true;

                Response.AddHeader("content-disposition", string.Format("attachment; filename={0}", "SaleReg_" + CurrDateTime + ".xls"));
                ...
                DataTable dt = default(DataTable);
                dt = MABL.BL.AppRptDP.Rpt_Get_Sales_Summary(var_SQLFilter, "");
                if (dt.Rows.Count > 0)
                {
                    gvSummary_Export.DataSource = dt;
                    gvSummary_Export.DataBind();
                }
                else
                {
                    gvSummary_Export.DataSource = null;
                    gvSummary_Export.DataBind();
                }



                gvSummary_Export.RenderControl(htw);
        ...
        private void ExportToExcelDetail_ExecutiveWiseSales()
        {
            try
            {
                string CurrDateTime = MaheshAF18.Utility.DateHelper.ConvertToDataBase(MaheshAF18.Utility.DateHelper.ConvertToForm(DateTime.Now)) +
                ...
                DataTable dt = default(DataTable);
                dt = MABL.BL.AppRptDP.Rpt_Get_Sales_Detail(var_SQLFilter, "");
                if (dt.Rows.Count > 0)
                {
                    gvDetail_Export.DataSource = dt;
                    gvDetail_Export.DataBind();
                }
                else
                {
                    gvDetail_Export.DataSource = null;
                    gvDetail_Export.DataBind();
                }



                gvDetail_Export.RenderControl(htw);
                Response.Write(sw.ToString());
                Response.End();
