...
{
    public partial class DCReg : Mahesh_Page
    {
        ...
        private void RefreshGrid_HDR(string SearchCriteria = "")
        {
            try
            {
                ...
                {
                      var_SQLFilter = " AND DC_DT BETWEEN  '" + Txt_FromDT.Text + "' AND  '" + Txt_ToDT.Text + "'";
                }
                DataTable dt = default(DataTable);
                dt = MABL.BL.AppRptDP.Rpt_Get_Sales_DC_Summary(var_SQLFilter, SearchCriteria);
                if (dt.Rows.Count > 0)
                {
        ...
        private void RefreshGrid_Item_DTL(string SearchCriteria = "")
        {
            try
            {
                ...
                {
                    var_SQLFilter = " AND DC_DT BETWEEN  '" + Dtl_Txt_FromDT.Text + "' AND  '" + Dtl_Txt_ToDT.Text + "'";
                }

                DataTable dt = default(DataTable);
                dt = MABL.BL.AppRptDP.Rpt_Get_Sales_DC_Detail(var_SQLFilter, SearchCriteria);
                if (dt.Rows.Count > 0)
                {
                    gvdtl.DataSource = dt;
                    gvdtl.DataBind();
